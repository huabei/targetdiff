...
from .graph_new import batch_hybrid_edge_connection
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_index, mask_ligand, e_w=None, fix_x=False):
        ...
        for i in range(self.num_h2x):
            ...
            # if not fix_x:
            delta_x = delta_x * mask_ligand[:, None]  # only ligand positions will be updated
            # rel_x = x[dst] - x[src]
            # dist = torch.norm(rel_x, p=2, dim=-1, keepdim=True)
...
class UniTransformerNew(nn.Module):
    ...
    def _build_share_blocks(self):
        ...
        for l_idx in range(self.num_layers):
            layer_l1 = AttentionLayerO2TwoUpdateNodeGeneral(
            ...
            layer_l2 = AttentionLayerO2TwoUpdateNodeGeneral(
                self.hidden_dim, self.n_heads, self.num_r_gaussian, self.edge_feat_dim, act_fn=self.act_fn,
                norm=self.norm,
                num_x2h=self.num_x2h, num_h2x=self.num_h2x, r_max=self.r_max, num_node_types=self.num_node_types,
                ew_net_type=self.ew_net_type, x2h_out_fc=self.x2h_out_fc, sync_twoup=self.sync_twoup,
            )
            layer_pl = AttentionLayerO2TwoUpdateNodeGeneral(
                self.hidden_dim, self.n_heads, self.num_r_gaussian, self.edge_feat_dim, act_fn=self.act_fn,
                norm=self.norm,
                num_x2h=self.num_x2h, num_h2x=self.num_h2x, r_max=self.r_max, num_node_types=self.num_node_types,
                ew_net_type=self.ew_net_type, x2h_out_fc=self.x2h_out_fc, sync_twoup=self.sync_twoup,
            )
            layer_p = AttentionLayerO2TwoUpdateNodeGeneral(
                self.hidden_dim, self.n_heads, self.num_r_gaussian, self.edge_feat_dim, act_fn=self.act_fn,
                norm=self.norm,
                num_x2h=self.num_x2h, num_h2x=self.num_h2x, r_max=self.r_max, num_node_types=self.num_node_types,
                ew_net_type=self.ew_net_type, x2h_out_fc=self.x2h_out_fc, sync_twoup=self.sync_twoup,
            )
            base_block.append(nn.ModuleList([layer_l1, layer_l2, layer_pl, layer_p]))
    ...
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False, time_step=None):
        ...
        for b_idx in range(self.num_blocks):
            ...
            for l_idx, layer in enumerate(self.base_block):
                delta_h_l1, delta_x_l2 = layer[0](h, x, edge_type, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x)
                delta_h_l2, delta_x_pl = layer[1](h, x, edge_type, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x)
                delta_h_pl, delta_x_p = layer[2](h, x, edge_type, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x)
                delta_h_p, delta_x = layer[3](h, x, edge_type, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x)
                h = h + delta_h_l1 + delta_h_l2 + delta_h_pl + delta_h_p
                x = x + delta_x_l2 + delta_x_pl + delta_x_p + delta_x
            all_x.append(x)
            all_h.append(h)
